...
*   CONVERT FROM FIXED (short BFP to int-64, RRE)
*   CONVERT FROM FIXED (long BFP to int-64, RRE) 
*   CONVERT FROM FIXED (extended BFP to int-64, RRE)  
*
...
*
         ...
         BAS   R13,CGEBRA     Convert values from fixed to short using rm options
*
         ...
         BAS   R13,CGDBRA     Convert values to fixed from long using rm options
*
         ...
         BAS   R13,CGXBRA     Convert values to fixed from extended using rm options
*
...
*
* Convert short BFP to integer-64 format.  A pair of results is generated
...
*
         ...
         STC   R1,(0*4)+3(0,R8)    Save condition code as low byte of FPCR
*
...
*
         ...
         CGEBRA R1,0,R0,B'0100'  FPC controlled rounding, inexact masked
...
*
         LFPC  FPCREGNT      Set all exceptions non-trappable, clear flags
         CGEBRA R1,1,R0,B'0000'  RNTA Round to nearest, ties away from zero
...
*
         LFPC  FPCREGNT      Set all exceptions non-trappable, clear flags
         CGEBRA R1,3,R0,B'0000'  RPS Round to prepare for shorter precision
...
*
         LFPC  FPCREGNT      Set all exceptions non-trappable, clear flags
         CGEBRA R1,4,R0,B'0000'  RNTE Round to nearest, ties to even
...
*
         LFPC  FPCREGNT      Set all exceptions non-trappable, clear flags
         CGEBRA R1,5,R0,B'0000'  RZ Round toward zero
...
*
         LFPC  FPCREGNT      Set all exceptions non-trappable, clear flags
         CGEBRA R1,6,R0,B'0000'  Round to +inf
...
*
         LFPC  FPCREGNT      Set all exceptions non-trappable, clear flags
         CGEBRA R1,7,R0,B'0000'  Round to -inf
...
*
         ...
         BR    R13           All converted; return.
*
* Convert long BFP inputs to integer-64.  A pair of results is generated
* for each input: one with all exceptions non-trappable, and the second 
* with all exceptions trappable.   The FPCR and condition code is stored 
* for each result.  
*
...
*
         ...
         LA    R7,16(0,R7)    Point to next integer-64 converted value pair
...
*
         LFPC  FPCREGNT      Set all exceptions non-trappable, clear flags
         SRNM  1             SET FPC to RZ, Round towards zero.  
         CGDBRA R1,0,R0,B'0100'  FPC controlled rounding, inexact masked
         STG   R1,0*8(0,R7)  Store integer-64 result
         STFPC 0(R8)         Store resulting FPC flags and DXC
         IPM   R1            Get condition code and program mask
         SRL   R1,28         Isolate CC in low order byte
...
*
         LFPC  FPCREGNT      Set all exceptions non-trappable, clear flags
         CGDBRA R1,1,R0,B'0000'  RNTA Round to nearest, ties away from zero
         STG   R1,4*8(0,R7)  Store integer-64 result
         STFPC 4*4(R8)       Store resulting FPC flags and DXC
         IPM   R1            Get condition code and program mask
         SRL   R1,28         Isolate CC in low order byte
...
*
         LFPC  FPCREGNT      Set all exceptions non-trappable, clear flags
         CGDBRA R1,3,R0,B'0000'  RPS Round to prepare for shorter precision
         STG   R1,5*8(0,R7)  Store integer-64 result
         STFPC 5*4(R8)       Store resulting FPC flags and DXC
         IPM   R1            Get condition code and program mask
         SRL   R1,28         Isolate CC in low order byte
...
*
         LFPC  FPCREGNT      Set all exceptions non-trappable, clear flags
         CGDBRA R1,4,R0,B'0000'  RNTE Round to nearest, ties to even
         STG   R1,6*8(0,R7)  Store integer-64 result
         STFPC 6*4(R8)       Store resulting FPC flags and DXC
         IPM   R1            Get condition code and program mask
         SRL   R1,28         Isolate CC in low order byte
...
*
         LFPC  FPCREGNT      Set all exceptions non-trappable, clear flags
         CGDBRA R1,5,R0,B'0000'  RZ Round toward zero
         STG   R1,7*8(0,R7)  Store integer-64 result
         STFPC 7*4(R8)       Store resulting FPC flags and DXC
         IPM   R1            Get condition code and program mask
         SRL   R1,28         Isolate CC in low order byte
...
*
         LFPC  FPCREGNT      Set all exceptions non-trappable, clear flags
         CGDBRA R1,6,R0,B'0000'  Round to +inf
         STG   R1,8*8(0,R7)  Store integer-64 result
         STFPC 8*4(R8)       Store resulting FPC flags and DXC
         IPM   R1            Get condition code and program mask
         SRL   R1,28         Isolate CC in low order byte
...
*
         LFPC  FPCREGNT      Set all exceptions non-trappable, clear flags
         CGDBRA R1,7,R0,B'0000'  Round to -inf
         STG   R1,9*8(0,R7)  Store integer-64 result
         STFPC 9*4(R8)       Store resulting FPC flags and DXC
         IPM   R1            Get condition code and program mask
         SRL   R1,28         Isolate CC in low order byte
...
*
         LFPC  FPCREGNT      Set all exceptions non-trappable, clear flags
         SRNM  1             SET FPC to RZ, Round towards zero.  
         CGXBRA R1,0,R0,B'0100'  FPC controlled rounding, inexact masked
         STG   R1,0*8(0,R7)  Store integer-64 result
         STFPC 0(R8)         Store resulting FPC flags and DXC
         IPM   R1            Get condition code and program mask
         SRL   R1,28         Isolate CC in low order byte
...
*
         LFPC  FPCREGNT      Set all exceptions non-trappable, clear flags
         CGXBRA R1,1,R0,B'0000'  RNTA Round to nearest, ties away from zero
         STG   R1,4*8(0,R7)  Store integer-64 result
         STFPC 4*4(R8)       Store resulting FPC flags and DXC
         IPM   R1            Get condition code and program mask
         SRL   R1,28         Isolate CC in low order byte
...
*
         LFPC  FPCREGNT      Set all exceptions non-trappable, clear flags
         CGXBRA R1,3,R0,B'0000'  RPS Round to prepare for shorter precision
         STG   R1,5*8(0,R7)  Store integer-64 result
         STFPC 5*4(R8)       Store resulting FPC flags and DXC
         IPM   R1            Get condition code and program mask
         SRL   R1,28         Isolate CC in low order byte
...
*
         LFPC  FPCREGNT      Set all exceptions non-trappable, clear flags
         CGXBRA R1,4,R0,B'0000'  RNTE Round to nearest, ties to even
         STG   R1,6*8(0,R7)  Store integer-64 result
         STFPC 6*4(R8)       Store resulting FPC flags and DXC
         IPM   R1            Get condition code and program mask
         SRL   R1,28         Isolate CC in low order byte
...
*
         LFPC  FPCREGNT      Set all exceptions non-trappable, clear flags
         CGXBRA R1,5,R0,B'0000'  RZ Round toward zero
         STG   R1,7*8(0,R7)  Store integer-64 result
         STFPC 7*4(R8)       Store resulting FPC flags and DXC
         IPM   R1            Get condition code and program mask
         SRL   R1,28         Isolate CC in low order byte
...
*
         LFPC  FPCREGNT      Set all exceptions non-trappable, clear flags
         CGXBRA R1,6,R0,B'0000'  Round to +inf
         STG   R1,8*8(0,R7)  Store integer-64 result
         STFPC 8*4(R8)       Store resulting FPC flags and DXC
         IPM   R1            Get condition code and program mask
         SRL   R1,28         Isolate CC in low order byte
...
*
         LFPC  FPCREGNT      Set all exceptions non-trappable, clear flags
         CGXBRA R1,7,R0,B'0000'  Round to -inf
         STG   R1,9*8(0,R7)  Store integer-64 result
         STFPC 9*4(R8)       Store resulting FPC flags and DXC
         IPM   R1            Get condition code and program mask
         SRL   R1,28         Isolate CC in low order byte
...
* int-64 to long or extended are always exact.  
*
SBFPIN   DS    0F                Inputs for short BFP testing
         ...
         DC    X'7F810000'         SNaN
         DC    X'7FC10000'         QNaN
         DC    X'5F000001'         positive max int-64 value plus 1.  (9223372036854775808 + 1)
         DC    X'DF000002'         negative max int-64 value minus 2.  (-9223372036854775808 - 2)
...
LBFPIN   DS    0F                Inputs for long BFP testing
         ...
         DC    X'43E0000000000000'   positive max int-64 value plus 1.  (9223372036854775808 + 1)
         DC    X'C3E0000000200000'   negative max int-64 value minus 2.  (-9223372036854775808 - 2)
         DS    0F                  required by asma for following EQU to work.
...
XBFPIN   DS    0D                Inputs for long BFP testing
         ...
         DC    X'403E0000000000000000000000000000'   positive max int-64 value plus 1.  (9223372036854775808 + 1)
         DC    X'C03E0000000200000000000000000000'   negative max int-64 value minus 2.  (-9223372036854775808 - 2)
...
*
SINTOUT  EQU   BFPCVTTF+X'1000'    Integer-64 values from short BFP, 16 planned, room for 16
SINTFLGS EQU   BFPCVTTF+X'1080'    FPC flags and DXC from short BFP, 16 planned, room for 16
SINTRMO  EQU   BFPCVTTF+X'1100'    Space for short rounding mode tests, room for 10 sets
SINTRMOF EQU   BFPCVTTF+X'1500'    Space for short rounding mode test flags, room for 10 sets
*
LINTOUT  EQU   BFPCVTTF+X'1700'    Integer-64 values from long BFP, 16 planned, room for 16
LINTFLGS EQU   BFPCVTTF+X'1780'    FPC flags and DXC from long BFP, 16 planned, room for 16
LINTRMO  EQU   BFPCVTTF+X'1800'    Space for long rounding mode tests, room for 10 sets
LINTRMOF EQU   BFPCVTTF+X'1C00'    Space for long  rounding mode test flags, room for 10 sets
*
XINTOUT  EQU   BFPCVTTF+X'1E00'    Integer-64 values from extended BFP, 16 planned, room for 16
XINTFLGS EQU   BFPCVTTF+X'1E80'    FPC flags and DXC from extended BFP, 16 planned, room for 16
XINTRMO  EQU   BFPCVTTF+X'1F00'    Space for extended rounding mode tests, room for 10 sets
XINTRMOF EQU   BFPCVTTF+X'2300'    Space for extended rounding mode test flags, room for 10 sets
*
